...
def test_run_views_analysis_empty(caplog):
...
def test_run_views_analysis_success(mock_get_stats, sample_df):
...
def test_main_file_not_found(mock_logger, mock_load):
